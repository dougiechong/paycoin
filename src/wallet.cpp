...
// dougcoin: optional setting to unlock wallet for block minting only;
...
// dougcoin: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // dougcoin: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            for (;;)
            {
                ...
                // dougcoin: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    if (!GetBoolArg("-avatar")) // dougcoin: not avatar mode
                    {
...
// dougcoin: create coin stake transaction
...
// dougcoin: check 'spent' consistency between wallet and txindex
// dougcoin: fix wallet spent state according to txindex
...
// dougcoin: disable transaction (only for coinstake)
